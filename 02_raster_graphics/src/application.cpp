...
void application::context_menu_select(int item) {
  ...
  case MA_TOOL_INCREASE_POINTS:
    if (dynamic_cast<star_tool*>(tool) != nullptr) {
      dynamic_cast<star_tool*>(tool)->increment_points();
    }
    break;

  // Decrease star points
  case MA_TOOL_DECREASE_POINTS:
    if (dynamic_cast<star_tool*>(tool) != nullptr) {
      dynamic_cast<star_tool*>(tool)->decrement_points();
    }
    break;
  }
...
void application::setup_context_menu() {
  ...
  glutAddMenuEntry(" Star                      (*) ", MA_TOOL_STAR);
  glutAddMenuEntry(" Increase star points      (+) ", MA_TOOL_INCREASE_POINTS);
  glutAddMenuEntry(" Decrease star points      (-) ", MA_TOOL_DECREASE_POINTS);
...
void application::key_down(unsigned char key, int x, int y) {
  ...
  case 's':
    ...
    break;
  case '*':
    context_menu_select(MA_TOOL_STAR);
    break;
  case '+':
    context_menu_select(MA_TOOL_INCREASE_POINTS);
    break;
  case '-':
    context_menu_select(MA_TOOL_DECREASE_POINTS);
    break;
  case 'r':
